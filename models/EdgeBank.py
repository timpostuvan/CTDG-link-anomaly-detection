...
def edge_bank_unlimited_memory(
    history_src_node_ids: np.ndarray, history_dst_node_ids: np.ndarray
):
    """
    ...
    """
    edge_memories = set(
        (history_src_node_id, history_dst_node_id)
        for history_src_node_id, history_dst_node_id in zip(
            history_src_node_ids, history_dst_node_ids
        )
    )
    return edge_memories


def edge_bank_time_window_memory(
    history_src_node_ids: np.ndarray,
    history_dst_node_ids: np.ndarray,
    history_node_interact_times: np.ndarray,
    time_window_mode: str,
    time_window_proportion: float,
):
    """
    ...
    # repeat_interval, which sets the time window size to average time intervals of repeated edges
        ...
        for history_src_node_id, history_dst_node_id, history_node_interact_time in zip(
            history_src_node_ids, history_dst_node_ids, history_node_interact_times
        ):
        ...
        for edge_tuple, edge_time_interval_list in edge_time_intervals.items():
            if len(edge_time_interval_list) > 1:
                sum_edge_time_intervals += np.mean(
                    [
                        edge_time_interval_list[i + 1] - edge_time_interval_list[i]
    ...
    memory_mask = np.logical_and(
        history_node_interact_times <= time_window_end_time,
...
def edge_bank_repeat_threshold_memory(
    history_src_node_ids: np.ndarray, history_dst_node_ids: np.ndarray
):
    """
    ...
    edge_memories = set(
        edge_tuple
        for edge_tuple, edge_frequency in edge_frequencies.items()
        if edge_frequency >= threshold
    )
    return edge_memories


def edge_bank_link_prediction(
    history_data: Data,
    edges: tuple,
    edge_bank_memory_mode: str,
    time_window_mode: str,
    time_window_proportion: float,
):
    """
    ...
    :param edges: tuple, edges with (src_node_ids, dst_node_ids)
    ...
    if edge_bank_memory_mode == "unlimited_memory":
        edge_memories = edge_bank_unlimited_memory(
            history_src_node_ids=history_data.src_node_ids,
            history_dst_node_ids=history_data.dst_node_ids,
        )
    elif edge_bank_memory_mode == "time_window_memory":
        edge_memories = edge_bank_time_window_memory(
            history_src_node_ids=history_data.src_node_ids,
            history_dst_node_ids=history_data.dst_node_ids,
            history_node_interact_times=history_data.node_interact_times,
    ...
    edge_probabilities = predict_link_probabilities(
        edge_memories=edge_memories, edges_tuple=edges
    )
