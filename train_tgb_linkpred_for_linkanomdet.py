"""
Train a TG model with validation on link prediction task and evaluate it with TGB package.
...
from models.modules import VanillaLinkPredictor
...
from evaluation.tgb_evaluate_linkpred import eval_linkpred_TGB
...
def anomaly_detection_single(args: SimpleNamespace):
    ...
    link_predictor = VanillaLinkPredictor(
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                train_metrics.append(
                    get_link_prediction_metrics(
                        predicts=predicts,
                        labels=labels,
                        structural_mask=torch.zeros_like(labels, dtype=torch.bool),
                        contextual_mask=torch.zeros_like(labels, dtype=torch.bool),
                        temporal_mask=torch.zeros_like(labels, dtype=torch.bool),
                    )
                )
...
def main():
    ...
    # If config does not contain ´test_anom_type´, set it to ´anom_type´.
    ...
    # Since the model is validated on link prediction task, no anomalies should be loaded in the validaton set.
    config.data.val_anom_type = ListConfig([None for _ in config.data.test_anom_type])
